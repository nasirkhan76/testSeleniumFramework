...
public class Test {
    ...
    private static HashMap<String,JSONObject> globalJSONObjects = new HashMap<String, JSONObject>();
    ...
    private static int pageLoadTimeOutWait = 1;   //Have to put this in cos of dodgy IE driver behaviour. .
    ...
    private static int pageLoadAfterActionTimeOutWait = 5;  //This is the timeout wait for after a "click" action is performed.
    ...
    public static JSONObject getGlobalJSONObject(String jsonName){
        if (globalJSONObjects.containsKey(jsonName)){
            return globalJSONObjects.get(jsonName);
        }
        else {
            return null;
        }
    ...
    public static int getJsonObjectLength(String jsonName){
        if (globalJSONObjects.containsKey(jsonName)){
            return globalJSONObjects.get(jsonName).length();
        }
        else {
            return 0;
        }
    ...
    public static Result returnJsonData(String jsonName, String jsonDataName, String jsonDataType){
        Result returnJsonDataResult = new Result();
        returnJsonDataResult.setErrorMessage("");
        returnJsonDataResult.setOutcome("PASS");
        returnJsonDataResult.setOutput("");
        String jsonData = "";
        try {
            if (globalJSONObjects.containsKey(jsonName)){
                JSONObject currentJSONObject = globalJSONObjects.get(jsonName);

                if (jsonDataType.equalsIgnoreCase("number")){
                    jsonData = String.valueOf(currentJSONObject.getDouble(jsonDataName));
                }
                else {
                    jsonData =  currentJSONObject.getString(jsonDataName);
                }
                returnJsonDataResult.setOutput(jsonData);
            }
            else {
                return null;
            }
            ...
            returnJsonDataResult.setErrorMessage(e.toString());
            returnJsonDataResult.setOutcome("FAIL");
        }
        return returnJsonDataResult;
    }
    ...
    public static void putGlobalJSONObject(String jsonName, JSONObject json){
        globalJSONObjects.put(jsonName, json);
    }
    ...
    public static void clearGlobalJSONObjects(){
        globalJSONObjects.clear();
    }
...
//        }
        ...
        //Process input parameteres to work out if they are text or script values:
//        if (isScriptValue(inputvalue))  {
//            Result callmethod = convertToScriptValue(inputvalue);
//            if (callmethod.getOutcome().equals("PASS")){
//                inputvalue = callmethod.getOutput();
//
//            }
//            else {
//                bcontinue = false;
//                actionResult.setErrorMessage(callmethod.getErrorMessage());
//            }
//
//        }
        if (action.contains("$")){
        ...
        if (inputvalue.contains("$")){
            if (action.toLowerCase().equals("evaluatenumber") || action.toLowerCase().equals("checknumericexpressiontrue")){
                ...
                if ((action.toLowerCase().equals("checkattribute") || action.toLowerCase().equals("setparameter") ||
                        action.toLowerCase().equals("getoptionalparameter") || action.toLowerCase().equals("checkjsondata") ||
                        action.toLowerCase().equals("checkjsonlength") ||
                        action.toLowerCase().equals("updatejsonobject")) && inputvalue.contains("=")) {
...
//            }
    ...
    public static void initialiseJSONObject(String JSONObjectName){
        JSONObject currentJSONObject = new JSONObject();
        globalJSONObjects.put(JSONObjectName,currentJSONObject);

    }


    public static String updateJSONNode(String JSONObjectName, String k, String  v, String Type){
        //TODO This may need to be more complicated but at moment just handles key/value pairs, and just strings and numbers . . :
        //Returns blank if successful. Otherwise returns the error message.
        JSONObject currentJSONObject;
        if (globalJSONObjects.containsKey(JSONObjectName)){
             currentJSONObject = globalJSONObjects.get(JSONObjectName);
        }
        else {
            Test.reportEvent("INFO", "Creating new json object called " + JSONObjectName);
        ...
        try {
            if (Type.equalsIgnoreCase("number")){
                ...
                currentJSONObject.put(k,num);
            }
            else {
                Test.reportEvent("INFO","Updating json object '" + JSONObjectName + "' with data: " + Type + " " + k + "=" + v);
    ...
    public static void openBrowser(){
        ...
        if(AppProperties.get("browser").equalsIgnoreCase("firefox")){
            ...
            Test.setDriver(new InternetExplorerDriver());
    ...
    public static void updateInitialTestResultFile(File fileToUpdate, String textForHeading, String textForRunLog){
        ...
        try {
            ...
            reportEvent("INFO", "Unable to update test results header and and run log link");
