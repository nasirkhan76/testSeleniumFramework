...
import com.automatedTest.extensions.LinkingExtension;
import com.automatedTest.extensions.ProduceTestSetResults;
import com.automatedTest.extensions.TestSetTestResult;
...
//import org.concordion.api.FullOGNL;  need this if you want to do stuff like #myobject=#myotherobject.itsMethod()
...
public class StepBase {
    ...
    private String resultsOutputTopLevel = resultsOutput + "\\com\\automatedTest";
    ...
    //private String currentResultLocation = resultsOutput + currentTestClassLocation.toString().substring(currentTestClassLocation.toString().indexOf("/com/automatedTest/")).replaceAll("/", "\\\\");
    ...
    public void tearDown() {
        ...
        if (testType.equals("INITIALCALLINGTEST")){
            Test.reportEvent("INFO", "Test " + currentScriptName + " completed. Test Status: " + testStatus);
            Test.reportEvent("DIVIDER", "");
            Test.reportEvent("DIVIDER", "");
            ...
            //Test.getDriver().quit();
              Test.quitAllDrivers(); //Close all driver sessions (more than one may exist if user has opened up another browser using openbrowser action)



        }
        if (testType.equals("CALLEDTEST")){
            Test.reportEvent("INFO", "--------Called script " + currentScriptName + " completed. Called Test Script Status: " + testStatus);
            Test.reportEvent("INFO", "Test continuing in calling test. . .");
            Test.reportEvent("DIVIDER", "");
        }
    ...
    public void setUpTest() throws Exception {
        currentTestStatus = "";
        Test.reportEvent("INFO", "Start of test '" + currentScriptName + "'");
...
//    }
    ...
    public Result performAction(String rowswitch, String comment, String bytype, String bytext, String byinnertext, String action, String inputvalue, String wait, String resultname) throws Exception {
        ...
        if (Test.getEndTest() == true) {
            ...
            performActionResult.setOutcome("FAIL");
            ...
            Test.reportEvent("INFO", "Step: '" + comment + "' was not performed. Row was commented out.");
            Test.reportEvent("DIVIDER", "");
        }
            ...
            Test.reportEvent("INFO", "Step: '" + comment + "' was not performed. Condition in rowswitch not met.");
            Test.reportEvent("DIVIDER", "");
        }
        else  {
        ...
        if (rowswitch.toLowerCase().contains("allowfail") && !performActionResult.getOutcome().equals("PASS")){
            Test.reportEvent("INFO", "Step failed but failure allowed for this step. Setting step result to PASS");
            performActionResult.setOutcome("PASS");
        }
...
//        if (Test.isScriptValue(val)){
//            //Process input parameteres to work out if they are text or script values:
//            Result callmethod = Test.convertToScriptValue(val);
//            if (callmethod.getOutcome().equals("PASS")){
//                v = callmethod.getOutput();
//
//            }
//            else {
//                bcontinue = false;
//                setParamResult.setOutcome("FAIL");
//                setParamResult.setErrorMessage(callmethod.getErrorMessage());
//
//            }
//        }
        if (val.contains("$")){
 ...
 //   public Delivery setUpDel(){
 //       Delivery deliv = new Delivery(driver);
 //       return deliv;
 //   }
    ...
    public Result callActionMethod(String methodname, HashMap<String,String> hm, String resultname) throws Exception {
        ...
        Test.reportEvent("INFO", "Starting step which calls method '" + methodname + "'");
        ...
        if (!callActionResult.getErrorMessage().equals("")){
            Test.reportEvent("INFO", callActionResult.getErrorMessage());
        }
        Test.reportEvent(callActionResult.getOutcome(), "Overall outcome of step calling method '" + methodname + "'");
        Test.reportEvent("DIVIDER", "");
